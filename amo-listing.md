...
The icon shows a white dot when local history is normal and a pause symbol when history deletion is active.
...
Tabs and webNavigation: used to detect page navigation and remove the current visited URL from local history.

Storage: used only to save whether the toolbar toggle is enabled.

Host access: used so normal http and https page visits can be detected and removed from local history while pause mode is enabled.
